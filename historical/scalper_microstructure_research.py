...
# Validates focused Phase-3 pockets on unseen later data.
# ============================================================
def research_phase4_oos(
    candles,
    micro_states,
    indicator_states,
    index_name,
    ...
    # Common/high-interest pockets discovered in Phase-3.
    pockets = [
        (
            "BULL_RANGE_MORNING_BASE_3C",
            "BULLISH",
            "RANGE_OTHER",
            "MORNING",
            "BASE",
    ...
    print()
    print("=" * 80)
    print("PHASE-4 OUT-OF-SAMPLE VALIDATION:", index_name)
    print("CHRONOLOGICAL SPLIT: 70% TRAIN | 30% TEST")
    print("TARGET 0.10% | SL 0.10%")
    print("=" * 80)
    print(
        "TOTAL CANDLES:",
        len(candles),
...
# ============================================================
# PHASE-5 FREQUENCY + WALK-FORWARD ROBUSTNESS
# Goal:
# 1. Measure practical UNIQUE signal frequency per trading day.
# 2. Avoid double-counting 3C/5C versions of the same entry.
# 3. Measure TARGET/day, SL/day, NONE/day and no-signal days.
# 4. Test the combined pocket portfolio across chronological folds.
# ============================================================
def research_phase5_frequency_robustness(
    candles,
    micro_states,
    indicator_states,
    index_name,
    ...
    # Entry pockets only.
    # 3C and 5C are evaluation windows, NOT separate entry signals.
    pockets = [
        (
            "BULL_RANGE_MORNING_BASE",
            "BULLISH",
            "RANGE_OTHER",
            "MORNING",
            "BASE",
    ...
    def get_day(candle):
        ...
        # Common historical timestamp formats normally begin YYYY-MM-DD.
        if len(value) >= 10:
            return value[:10]

        return value

    # Build one unique entry record per candle/pocket.
    entries = []
    ...
    for i in range(len(candles)):
        ...
        for (
            ...
            for window in evaluation_windows:
                outcomes[window] = target_before_stop(
                    candles,
                    i,
                    direction,
                    window,
            ...
            # One candle should belong to only one selected pocket.
            break
    ...
    print(
        "TARGET 0.10% | SL 0.10% | UNIQUE ENTRY SIGNALS"
    )
    print("=" * 80)
    ...
    print()
    print("FULL-SAMPLE PRACTICAL FREQUENCY")
    print("-" * 80)
    ...
    for window in evaluation_windows:
        ...
        for entry in entries:
            signal_days.add(entry["day"])

            outcome = entry["outcomes"][window]
        ...
        no_signal_days = (
            total_days - len(signal_days)
        )

        no_signal_pct = (
            no_signal_days / total_days * 100
            if total_days
            else 0
        )
        ...
        print(
            ...
            f"T {target} ({target_per_day:.2f}/DAY) | "
            f"S {stop} ({stop_per_day:.2f}/DAY) | "
            f"NONE {none} ({none_per_day:.2f}/DAY) | "
            f"WIN {win_rate:.2f}% | "
            f"NO-SIGNAL DAYS {no_signal_days}/{total_days} "
            f"({no_signal_pct:.2f}%)"
        )

    print()
    print("POCKET-WISE FREQUENCY")
    print("-" * 80)
    ...
    for pocket in pockets:
        ...
        print(
            f"{pocket_name:30s} | "
            f"SIGNALS {len(pocket_entries)} | "
            f"SIGNALS/DAY {signals_per_day:.3f} | "
            f"ACTIVE DAYS {len(pocket_days)}"
        )

    # --------------------------------------------------------
    # Walk-forward folds.
    # Each fold is a chronological slice of the full history.
    # This checks whether frequency and edge persist over time.
    # --------------------------------------------------------
    print()
    print("CHRONOLOGICAL WALK-FORWARD FOLDS")
    print("-" * 80)
    ...
    for fold in range(fold_count):
        start = int(
            candle_count * fold / fold_count
        )

        end = int(
            candle_count * (fold + 1) / fold_count
        )
        ...
        for i in range(start, end):
            ...
            if day not in fold_days:
                fold_days.append(day)
        ...
        for window in evaluation_windows:
            ...
            print(
                ...
                f"WIN {win_rate:.2f}% | "
                f"RESOLVED {resolved_pct:.2f}% | "
                f"NO-SIGNAL DAYS {no_signal_days}"
            )
...
def research_index(api, index_name):
    ...
    # ===== PHASE-4 OUT-OF-SAMPLE VALIDATION =====
    research_phase4_oos(candles, micro_states, indicator_states, index_name)

    # ===== PHASE-5 FREQUENCY + WALK-FORWARD ROBUSTNESS =====
    research_phase5_frequency_robustness(
        candles,
        micro_states,
        indicator_states,
        index_name,
