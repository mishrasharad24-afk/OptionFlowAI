...
def part10_main():
    ...
    # ================================================================
    # PART 34 - FULL 180-DAY SPOT VWAP + MOMENTUM FOLLOW-THROUGH DIAGNOSTIC
    # Reuses already-fetched 180-day spot candles.
    # No additional API fetch.
    # Pre-entry diagnostic only.
    # ================================================================

    print("\n" + "=" * 120)
    print("PART 34 - FULL 180-DAY SPOT VWAP + MOMENTUM FOLLOW-THROUGH DIAGNOSTIC")
    print("BASE: PART 24 CAPPED PE REVERSAL EVENTS")
    print("DATA: FULL 180-DAY NIFTY SPOT HISTORY ALREADY LOADED")
    print("OUTCOME MODEL: TARGET 0.12% | SL 0.10% | HORIZON 5 CANDLES")
    print("PURPOSE: COMPARE WIN vs LOSS/TIME_EXIT USING PRE-ENTRY FEATURES")
    print("RULE: DIAGNOSTIC ONLY | NO FINAL FILTER SELECTION | NO FUTURE LEAKAGE")
    print("=" * 120)

    # ------------------------------------------------------------
    # Build full 180-day intraday SPOT VWAP map from existing candles.
    # ------------------------------------------------------------

    p34_by_time = {}
    p34_current_day = None
    p34_cum_pv = 0.0
    p34_cum_v = 0.0
    p34_prev = None
    ...
    for x in candles:
        ...
        p34_cum_pv += typical * volume
        p34_cum_v += volume

        # NIFTY spot historical volume may be zero/unavailable.
        # In that case use cumulative typical-price mean as VWAP proxy.
        if p34_cum_v > 0:
            p34_vwap = p34_cum_pv / p34_cum_v
        else:
            same_day = [
                y for y in candles
                if y["dt"].date() == day
        ...
        p34_row = dict(x)
        p34_row["vwap"] = p34_vwap

        if p34_prev is not None:
            p34_row["prev_close"] = p34_prev["c"]
            p34_row["prev_vwap"] = p34_prev["vwap"]
        else:
            p34_row["prev_close"] = None
            p34_row["prev_vwap"] = None

        p34_by_time[x["dt"]] = p34_row
        p34_prev = p34_row
    ...
    # ------------------------------------------------------------
    # Match full 180-day Part 24 events to spot VWAP.
    # ------------------------------------------------------------

    p34_matched = []

    for e in p24_capped_events:
        spot = p34_by_time.get(e["dt"])
        ...
        x["spot_high"] = spot["h"]
        x["spot_low"] = spot["l"]
        x["spot_vwap"] = spot["vwap"]

        if spot["vwap"]:
            x["spot_vwap_distance_pct"] = (
                (spot["c"] - spot["vwap"])
                / spot["vwap"]
                * 100.0
            )
        else:
            x["spot_vwap_distance_pct"] = 0.0

        x["spot_above_vwap"] = (
            spot["c"] > spot["vwap"]
        )

        p34_matched.append(x)

    print(
        "PART24 EVENTS:",
        len(p24_capped_events),
        "| MATCHED 180D EVENTS:",
        len(p34_matched),
    )

    # ------------------------------------------------------------
    # Primary diagnostic setup.
    # ------------------------------------------------------------

    p34_events = [
        e for e in p34_matched
        if e["spot_above_vwap"]
        and e.get("features", {}).get(
            "MOMENTUM_DOWN",
            False,
        )
    ]
    ...
    print(
        "ABOVE_SPOT_VWAP + MOMENTUM_DOWN EVENTS:",
        len(p34_events),
    )

    # ------------------------------------------------------------
    # Part 34 local reporting helper.
    # Keeps Part 34 independent from Part 30 runtime definition.
    # ------------------------------------------------------------

    def p34_report(label, events):
        m34 = p16_metrics(events)
        ...
        print(
            ...
            "| WINS", m34["wins"],
            "| LOSSES", m34["losses"],
            "| TIME_EXIT", m34["time_exits"],
            "| WIN_RATE", round(m34["win_rate"], 2), "%",
            "| AVG_PNL", round(m34["avg_pnl"], 4), "%",
            "| PF", round(m34["pf"], 3),
        )
    ...
    for e in p34_events:
        ...
        if result == "WIN":
            ...
            p34_time_exits.append(e)

    print("\nEXACT OUTCOME SUMMARY")
    p34_report(
        "FULL_180D ABOVE_SPOT_VWAP + MOMENTUM_DOWN",
        p34_events,
    )

    print(
        "WINS:", len(p34_wins),
        "| LOSSES:", len(p34_losses),
        "| TIME_EXITS:", len(p34_time_exits),
    )

    # ------------------------------------------------------------
    # Pre-entry Boolean feature comparison.
    # ------------------------------------------------------------
    ...
    for feature_name in p34_features:

        with_feature = [
            e for e in p34_events
            if e.get("features", {}).get(
                feature_name,
                False,
            )
        ]
    ...
    p34_buckets = (
        ...
        (
            "ABOVE_0.05_TO_0.10",
            lambda d: 0.05 <= d < 0.10,
        ),
        (
            "ABOVE_0.10_TO_0.20",
            lambda d: 0.10 <= d < 0.20,
        ),
        (
            "ABOVE_0.20_PLUS",
            lambda d: d >= 0.20,
    ...
    for bucket_name, condition in p34_buckets:

        subset = [
            e for e in p34_events
            if condition(
                e["spot_vwap_distance_pct"]
            )
        ]
    ...
    # ------------------------------------------------------------
    # Event-level failure inspection.
    # Print only LOSS and TIME_EXIT to keep output manageable.
    # ------------------------------------------------------------

    print("\nLOSS / TIME_EXIT EVENT DIAGNOSIS")
    ...
    for i, e in enumerate(p34_failed, 1):
    ...
    # ================================================================
    # PART 32 - VWAP + MOMENTUM EVENT-LEVEL DIAGNOSIS
    # ================================================================

    print("\n" + "=" * 120)
    print("PART 32 - VWAP + MOMENTUM EVENT-LEVEL DIAGNOSIS")
    print("BASE: ABOVE_VWAP + MOMENTUM_DOWN")
    print("PURPOSE: INSPECT INDIVIDUAL TRADES AND COMPARE EMA FEATURES")
    print("RULE: DIAGNOSTIC ONLY | NO FINAL FILTER SELECTION")
    print("=" * 120)

    p32_events = [
        e for e in p29_details
        if e["vwap_distance_pct"] > 0.0
        and e.get("features", {}).get("MOMENTUM_DOWN", False)
    ]

    print("\nQUALIFYING EVENTS:", len(p32_events))

    for i, e in enumerate(p32_events, 1):
        features = e.get("features", {})

        print("\n" + "-" * 100)
        print("EVENT", i)
        print("DATE/TIME:", e.get("dt"))
        print(
            "VWAP_DISTANCE_PCT:",
            round(e.get("vwap_distance_pct", 0.0), 4),
        )
        print(
            "FUT_CLOSE:",
            e.get("fut_close"),
        ...
        print(
            "EMA_BREAK:",
            features.get("EMA_BREAK", False),
            "| EMA_REJECTION:",
            features.get("EMA_REJECTION", False),
            "| MOMENTUM_DOWN:",
            features.get("MOMENTUM_DOWN", False),
        )
        print(
            "EMA_BEAR:",
            features.get("EMA_BEAR", False),
            "| BREAK_PREV_LOW:",
            features.get("BREAK_PREV_LOW", False),
            "| CLOSE_NEAR_LOW:",
            features.get("CLOSE_NEAR_LOW", False),
        )
        print(
            "PRIOR_UP_5:",
            features.get("PRIOR_UP_5", False),
            "| BEARISH_ENGULF:",
            features.get("BEARISH_ENGULF", False),
        )
    ...
    print("\nSUMMARY")
    p30_report("ABOVE_VWAP + MOMENTUM_DOWN", p32_events)

    p32_with_ema_break = [
        e for e in p32_events
        if e.get("features", {}).get("EMA_BREAK", False)
    ]

    p32_without_ema_break = [
        e for e in p32_events
        if not e.get("features", {}).get("EMA_BREAK", False)
    ]

    print("\nEMA BREAK COMPARISON")
    p30_report("WITH_EMA_BREAK", p32_with_ema_break)
    p30_report("WITHOUT_EMA_BREAK", p32_without_ema_break)

    print("\nPART 32 COMPLETE")


    # ================================================================
    # PART 33 - VWAP + MOMENTUM EXACT OUTCOME DIAGNOSIS
    # ================================================================

    print("\n" + "=" * 120)
    print("PART 33 - VWAP + MOMENTUM EXACT OUTCOME DIAGNOSIS")
    print("BASE: ABOVE_VWAP + MOMENTUM_DOWN")
    print("OUTCOME MODEL: TARGET 0.12% | SL 0.10% | HORIZON 5 CANDLES")
    print("PURPOSE: IDENTIFY EXACT WIN / LOSS / TIME_EXIT EVENTS")
    print("=" * 120)
    ...
    print("\nQUALIFYING EVENTS:", len(p33_events))

    for i, e in enumerate(p33_events, 1):
        features = e.get("features", {})
        ...
        print("FORWARD CANDLES:")
        for j, candle in enumerate(e["future"], 1):
            print(" ", j, candle)

    print("\nSUMMARY")
    p30_report("ABOVE_VWAP + MOMENTUM_DOWN", p33_events)
