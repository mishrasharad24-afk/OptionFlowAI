...
def fetch_spot_history(m):
    ...
    while cursor < end:
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                chunk_rows = get_candles(response)
        ...
        if chunk_rows:
            ...
            print(
                "SPOT FETCH FAILED AFTER RETRIES",
                cursor.strftime("%Y-%m-%d"),
                "TO",
                chunk_end.strftime("%Y-%m-%d"),
            )
    ...
    print(
        "SPOT FETCH TOTAL UNIQUE CANDLES:",
        len(unique),
    )
...
def new_matrix_stats():
    return {
        side: {
            ...
            for target, sl in TARGET_SL_MATRIX
        }
        for side in ("CE", "PE")
    }
...
def check_target_sl(
    ...
    # Scan all remaining candles
    # after signal candle
    for candle in data[pos + 1:]:
        ...
        # If both touch inside same 5m candle,
