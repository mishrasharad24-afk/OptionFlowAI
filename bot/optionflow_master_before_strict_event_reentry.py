...
def scanner(token):
    ...
    while True:
        ...
        for name in ["NIFTY","SENSEX"]:
            ...
            # ===== EVENT-BASED SAME-SIDE RE-ENTRY =====
            # 10 minutes is only a minimum gap.
            # The active trade is NOT unlocked by time.
            # Re-entry requires:
            # 1) minimum 10 minutes from previous signal
            # 2) premium pulls back at least 5% below previous entry
            # 3) fresh pullback low is tracked
            # 4) premium recovers 3% from that fresh low
            # 5) flow + AI direction are still aligned

            reentry_unlock_now = False

            if (
                current_trade in ("CE", "PE")
                and memory[name].get("entry_time", 0) > 0
                and time.time() - memory[name]["entry_time"] >= 600
            ):
                active_ltp = ce_ltp if current_trade == "CE" else pe_ltp
                active_confirmed = (
                    ce_entry_confirmed
                    if current_trade == "CE"
                ...
                previous_entry = memory[name].get("entry_price", 0)

                # A real pullback must occur AFTER the previous signal.
                if (
                    previous_entry > 0
                    ...
                    if (
                        ...
                        print(
                            "[REENTRY PULLBACK ARMED]",
                            name,
                            "| SIDE", current_trade,
                            "| ENTRY", previous_entry,
                            "| FRESH LOW", active_ltp
                        )
                    else:
                ...
                if (
                    ...
                    # Continue tracking a new lower pullback low.
                    memory[name]["reentry_low"] = min(
                        fresh_low,
                        active_ltp
                    )
                    ...
                    if (
                        ...
                        send(
                            ...
                            f"SYMBOL: {ce if current_trade == 'CE' else pe}\n"
                            f"PRICE: {active_ltp}\n"
                            f"FRESH PULLBACK LOW: {fresh_low:.2f}\n"
                            f"ENTRY TYPE: FRESH PULLBACK RECOVERY\n"
                        ...
                        # Start a new minimum-gap cycle from this re-entry.
