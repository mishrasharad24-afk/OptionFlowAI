...
def scanner(token):
    ...
    while True:
        ...
        for name in ["NIFTY","SENSEX"]:
            ...
            # ===== 5 MINUTE RE-ENTRY UNLOCK =====
            # Manual SL mode: release internal trade lock after 5 minutes.
            if current_trade in ("CE", "PE") and memory[name].get("entry_time", 0) > 0 and time.time() - memory[name]["entry_time"] >= 300:
                print("[5MIN REENTRY UNLOCK]", name, "| SIDE", current_trade)
