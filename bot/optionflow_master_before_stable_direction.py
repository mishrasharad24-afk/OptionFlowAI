...
def scanner(token):
    ...
    while True:
        ...
        for name in ["NIFTY","SENSEX"]:
            ...
            if current_trade == "CE" and pe_score >= 80 and ai_direction == "BEARISH":
                print("[EXIT DEBUG]", name, "| TRADE CE | CE_SCORE", ce_score, "| PE_SCORE", pe_score, "| AI_DIR", ai_direction)
                send(
                    f"⚠️ EXIT {name} CE\n"
                    f"REASON: PE FLOW {pe_score} + AI {ai_direction}\n"
            ...
            elif current_trade == "PE" and ce_score >= 80 and ai_direction == "BULLISH":
                print("[EXIT DEBUG]", name, "| TRADE PE | CE_SCORE", ce_score, "| PE_SCORE", pe_score, "| AI_DIR", ai_direction)
                send(
                    f"⚠️ EXIT {name} PE\n"
                    f"REASON: CE FLOW {ce_score} + AI {ai_direction}\n"
                    f"CE SCORE: {ce_score} | PE SCORE: {pe_score}\n"
                    f"COOLDOWN: 5 MIN"
                )
                memory[name]["trade"] = None
            ...
            # ENTRY only if there was NO active trade at start of this cycle
                ...
                if ce_score >= 70 and ai_direction == "BULLISH":
                    send(
                ...
                elif pe_score >= 70 and ai_direction == "BEARISH":
                    send(
