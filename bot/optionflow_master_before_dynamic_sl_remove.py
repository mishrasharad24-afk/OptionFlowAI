...
def scanner(token):
    ...
    while True:
        ...
        for name in ["NIFTY","SENSEX"]:
            ...
            # ===== DYNAMIC PREMIUM SL / TRAILING ENGINE =====
            # SL requires 2 consecutive cycles below stop (~10 sec).
            # This avoids exiting on one random 5-second premium tick.

            dynamic_sl_exit = False

            trade_symbol = memory[name].get("trade_symbol")
            trade_ltp = ltp(headers, trade_symbol) if trade_symbol else None

            if current_trade in ("CE", "PE") and not trade_ltp:
                print("[TRADE LTP MISSING]", name, "| SIDE", current_trade, "| SYMBOL", trade_symbol)


            if current_trade == "CE":
                memory[name]["trade_high"] = max(
                    memory[name].get("trade_high", 0),
                    trade_ltp
                )

                entry_price = memory[name].get("entry_price", 0)
                trade_high = memory[name].get("trade_high", 0)

                if entry_price > 0:
                    gain_pct = ((trade_high - entry_price) / entry_price) * 100

                    # Initial protection: 8% below entry.
                    new_sl = entry_price * 0.92

                    # Trail progressively as premium expands.
                    if gain_pct >= 30:
                        new_sl = max(new_sl, trade_high * 0.90)
                    elif gain_pct >= 20:
                        new_sl = max(new_sl, entry_price * 1.08)
                    elif gain_pct >= 12:
                        new_sl = max(new_sl, entry_price * 1.03)
                    elif gain_pct >= 7:
                        new_sl = max(new_sl, entry_price)

                    memory[name]["dynamic_sl"] = max(
                        memory[name].get("dynamic_sl", 0),
                        new_sl
                    )

                    if trade_ltp <= memory[name]["dynamic_sl"]:
                        memory[name]["sl_hit_count"] += 1
                    else:
                    ...
                    print(
                        ...
                        "| HIGH", round(trade_high, 2),
                        "| LTP", round(trade_ltp, 2),
                        "| SL", round(memory[name]["dynamic_sl"], 2),
                        "| HIT_COUNT", memory[name]["sl_hit_count"],
                    )

                    if memory[name]["sl_hit_count"] >= 2:
                        dynamic_sl_exit = True

            elif current_trade == "PE":
                memory[name]["trade_high"] = max(
                    memory[name].get("trade_high", 0),
                    trade_ltp
                )
            ...
            # Dynamic SL exit comes before opposite-flow exit.
            if current_trade in ("CE", "PE") and dynamic_sl_exit:
                exit_ltp = trade_ltp if current_trade == "CE" else trade_ltp

                send(
                    f"🛡️ DYNAMIC SL EXIT {name} {current_trade}\n"
                    f"EXIT PRICE: {exit_ltp:.2f}\n"
                    f"ENTRY PRICE: {memory[name]['entry_price']:.2f}\n"
                    f"TRAIL SL: {memory[name]['dynamic_sl']:.2f}\n"
                    f"RE-ENTRY: SAME-SIDE PULLBACK ALLOWED AFTER 30 SEC"
                )

                memory[name]["last_exit_side"] = current_trade
                memory[name]["last_exit_reason"] = "DYNAMIC_SL"
            ...
            elif current_trade == "CE" and pe_flow_confirmed and ai_direction == "BEARISH" and stable_direction == "BEARISH":
