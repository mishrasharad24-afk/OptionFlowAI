...
# ===== V3 ENGINES =====
from engines.current_day_history import CurrentDayHistory
from engines.market_memory import MarketMemory
from engines.direction_engine import DirectionEngine
from engines.opening_engine import OpeningEngine
...
from flow_state import save_flow_state, load_flow_state
...
def scanner(token):
    ...
    # ===== V3 SMART ENGINE =====
    history_engine = CurrentDayHistory()
    market_memory_engine = MarketMemory()
    direction_engine = DirectionEngine()
    opening_engine = OpeningEngine()
    price_action_engine = PriceActionEngine()

    history_engine.bootstrap()
    market_memory_engine.bootstrap()

    print("V3 SMART ENGINE READY")
    send("🧠 V3 SMART ENGINE READY")
    ...
    while True:
        ...
        for name in ["NIFTY","SENSEX"]:
            # ===== V3 MARKET CONTEXT =====
            if name == "NIFTY":
                market_symbol = "NSE:NIFTY 50"
            else:
                market_symbol = "BSE:SENSEX"
            ...
            if direction_state.get("trend") == "UPTREND":
                ai_score += 20
            elif direction_state.get("trend") == "DOWNTREND":
                ai_score -= 20

            if opening_state.get("confidence", 0) >= 70:
                ai_score += 10

            if price_state.get("bottom"):
                ai_score += 15

            if price_state.get("top"):
                ai_score -= 15

            if price_state.get("bos"):
                ai_score += 15
            ...
            if direction_state.get("trend") == "UPTREND":
                ai_reason.append("UPTREND")

            if direction_state.get("trend") == "DOWNTREND":
                ai_reason.append("DOWNTREND")

            if price_state.get("bos"):
                ai_reason.append("BOS")

            if price_state.get("bottom"):
                ai_reason.append("BOTTOM_ZONE")

            if price_state.get("top"):
                ai_reason.append("TOP_ZONE")
            ...
            # ===== V3 HISTORY CONTEXT =====
            market_history = history_engine.get(spot_symbol)

            candles_5m = market_history.get("history_5m", [])
            candles_15m = market_history.get("history_15m", [])

            direction_state = direction_engine.calculate(
                name,
                candles_5m,
                candles_15m
            )
            opening_state = opening_engine.analyze(
                name,
                candles_5m
            )

            price_state = price_action_engine.analyze(
                name,
                candles_5m
            )
            # ===== SAFE AI STATE =====
            ...
            if ce_power >= 2 and pe_falling >= 1:
                ce_score += 50

            if pe_power >= 2 and ce_falling >= 1:
                pe_score += 50
            ...
            # Strong flow must persist for consecutive cycles.
            flow_threshold = 70
            if name == "SENSEX":
                flow_threshold = 50

            if ce_score >= flow_threshold and ce_move > 0:
                memory[name]["ce_flow_count"] += 1
            else:
                memory[name]["ce_flow_count"] = max(
                    0,
                    memory[name]["ce_flow_count"] - 1,
                )

            if pe_score >= flow_threshold and pe_move > 0:
                memory[name]["pe_flow_count"] += 1
            else:
                memory[name]["pe_flow_count"] = max(
                    0,
                    memory[name]["pe_flow_count"] - 1,
                )
            ...
            print(
                "[FLOW DETAIL]",
                name,
                "| CE_FLOW", ce_flow_confirmed,
                "| CE_FAST", ce_fast_confirmed,
                "| CE_ENTRY", ce_entry_confirmed,
                "| PE_FLOW", pe_flow_confirmed,
                "| PE_FAST", pe_fast_confirmed,
                "| PE_ENTRY", pe_entry_confirmed,
            )


            # ===== SAFE TRADE STATE / PULLBACK ENTRY =====
            current_trade = memory[name]["trade"]
            # ===== STRICT EVENT-BASED SAME-SIDE RE-ENTRY =====
            # RULES:
            # 1) First 10 minutes after a signal are cooldown only.
            #    Any pullback during cooldown is completely ignored.
            # 2) Only AFTER 10 minutes can a NEW pullback start.
            # 3) Re-entry state is tied to the exact option symbol.
            # 4) Premium must fall at least 5% from previous entry.
            # 5) After fresh low, recovery must be between 3% and 8%.
            # 6) Flow + AI direction must remain aligned.
            # 7) Timer NEVER clears the active trade.

            reentry_unlock_now = False

            if current_trade in ("CE", "PE"):
                ...
                previous_entry = memory[name].get(
                    "entry_price",
                    0
                )
                ...
                elapsed = (
                    time.time()
                    - memory[name].get("entry_time", 0)
                )

                # During the first 10 minutes, absolutely no
                # pullback/re-entry state is retained.
                if elapsed < 600:
                    memory[name]["reentry_armed"] = False
                    memory[name]["reentry_low"] = 0
                    memory[name]["reentry_side"] = None
                    memory[name]["reentry_symbol"] = None
                    memory[name]["reentry_baseline"] = 0
                    memory[name]["reentry_baseline_symbol"] = None
                    memory[name]["reentry_pullback_seen"] = False

                else:
                    # If ATM strike/symbol changes, discard any
                    # low belonging to the previous option contract.
                    if (
                        memory[name].get("reentry_symbol") is not None
                        and memory[name]["reentry_symbol"] != active_symbol
                    ):
                        print(
                            "[REENTRY SYMBOL RESET]",
                            name,
                            "| OLD",
                            memory[name]["reentry_symbol"],
                            "| NEW",
                            active_symbol
                        )
                    ...
                    # TRUE POST-COOLDOWN BASELINE:
                    # The first observed premium AFTER cooldown becomes
                    # the baseline. No re-entry can arm in this same cycle.
                    if (
                        memory[name].get("reentry_baseline", 0) <= 0
                        or memory[name].get("reentry_baseline_symbol") != active_symbol
                    ):
                        memory[name]["reentry_baseline"] = active_ltp
                        memory[name]["reentry_baseline_symbol"] = active_symbol
                        ...
                        print(
                            ...
                            "| BASELINE",
                            active_ltp,
                            "| WAITING FOR NEW 5% PULLBACK"
                        )

                        fresh_pullback = False

                    else:
                        baseline = memory[name]["reentry_baseline"]

                        # Pullback must happen AFTER baseline was established.
                        fresh_pullback = (
                            baseline > 0
                            and active_ltp <= baseline * 0.95
                            and direction_ok
                        )
                    ...
                    if fresh_pullback:
                        ...
                        if (
                            not memory[name]["reentry_armed"]
                            or memory[name]["reentry_side"] != current_trade
                            or memory[name].get("reentry_symbol") != active_symbol
                        ):
                            memory[name]["reentry_armed"] = True
                            memory[name]["reentry_side"] = current_trade
                            memory[name]["reentry_symbol"] = active_symbol
                            memory[name]["reentry_low"] = active_ltp
                            memory[name]["reentry_pullback_seen"] = True

                            print(
                                "[POST-COOLDOWN FRESH PULLBACK]",
                                name,
                                "| SIDE",
                                current_trade,
                                "| SYMBOL",
                            ...
                            memory[name]["reentry_low"] = min(
                                memory[name]["reentry_low"],
                                active_ltp
                            )

                    # Recovery is valid only for the same exact symbol.
                    if (
                        memory[name]["reentry_armed"]
                        and memory[name]["reentry_pullback_seen"]
                        and memory[name]["reentry_side"] == current_trade
                        and memory[name].get("reentry_symbol") == active_symbol
                        and memory[name]["reentry_low"] > 0
                    ):
                        ...
                        fresh_low = memory[name]["reentry_low"]

                        # Entry only in the 3%-8% recovery zone.
                        reentry_recovery = (
                            active_ltp >= fresh_low * 1.03
                            and active_ltp <= fresh_low * 1.08
                        )

                        if (
                            reentry_recovery
                            and active_confirmed
                            and direction_ok
                        ):
                            send(
                                f"♻️ {name} {current_trade} RE-ENTRY SIGNAL\n"
                                f"SYMBOL: {active_symbol}\n"
                                f"PRICE: {active_ltp}\n"
                                f"FRESH PULLBACK LOW: {fresh_low:.2f}\n"
                                f"ENTRY TYPE: POST-COOLDOWN FRESH PULLBACK RECOVERY\n"
                                f"FLOW SCORE: {ce_score if current_trade == 'CE' else pe_score}\n"
                            ...
                            # New signal starts a completely new
                            # 10-minute cooldown cycle.
                            memory[name]["entry_price"] = active_ltp
                            memory[name]["entry_time"] = time.time()
                            memory[name]["trade_symbol"] = active_symbol
            ...
            # ===== ARM DIRECTION FIRST, DO NOT CHASE FIRST SIGNAL =====
            elif current_trade is None and not reentry_unlock_now and (
                ...
                if memory[name]["ce_entry_armed"] and ai_direction == "BULLISH":
                    memory[name]["ce_arm_low"] = min(memory[name]["ce_arm_low"], ce_ltp)

                if memory[name]["pe_entry_armed"] and ai_direction == "BEARISH":
                    memory[name]["pe_arm_low"] = min(memory[name]["pe_arm_low"], pe_ltp)

                # ===== 2-STAGE FIRST ENTRY RECOVERY CONFIRMATION =====
                # Stage 1:
                # Premium must recover at least 3% from the pullback low.
                #
                # Stage 2:
                # After crossing the recovery level, premium must HOLD
                # above that level for 20 seconds.
                #
                # If premium falls back below the recovery level before
                # confirmation completes, the hold timer is reset.
                # This filters temporary bounce/rejection entries.

                ce_recovery_level = (
                    memory[name]["ce_arm_low"] * 1.03
                    if memory[name]["ce_entry_armed"]
                    and memory[name]["ce_arm_low"] > 0
                    else 0
                )

                pe_recovery_level = (
                    memory[name]["pe_arm_low"] * 1.03
                    if memory[name]["pe_entry_armed"]
                ...
                if (
                    ...
                    and ce_recovery_level > 0
                    and ce_ltp >= ce_recovery_level
                    and ai_direction == "BULLISH"
                ):
                    if memory[name]["ce_recovery_since"] <= 0:
                        memory[name]["ce_recovery_since"] = time.time()
                        memory[name]["ce_recovery_level"] = ce_recovery_level

                        print(
                            "[CE RECOVERY HOLD START]",
                            name,
                            "| LOW", memory[name]["ce_arm_low"],
                            "| LEVEL", ce_recovery_level,
                            "| PRICE", ce_ltp
                        )
                    ...
                    if memory[name]["ce_recovery_since"] > 0:
                        print(
                            "[CE RECOVERY HOLD RESET]",
                            name,
                            "| PRICE", ce_ltp
                        )

                    memory[name]["ce_recovery_since"] = 0
                    memory[name]["ce_recovery_level"] = 0

                # ----- PE RECOVERY HOLD -----
                if (
                    memory[name]["pe_entry_armed"]
                    and pe_recovery_level > 0
                    and pe_ltp >= pe_recovery_level
                    and ai_direction == "BEARISH"
                ):
                    if memory[name]["pe_recovery_since"] <= 0:
                        memory[name]["pe_recovery_since"] = time.time()
                        memory[name]["pe_recovery_level"] = pe_recovery_level

                        print(
                            "[PE RECOVERY HOLD START]",
                            name,
                            "| LOW", memory[name]["pe_arm_low"],
                            "| LEVEL", pe_recovery_level,
                            "| PRICE", pe_ltp
                        )
                    ...
                    if memory[name]["pe_recovery_since"] > 0:
                        print(
                            "[PE RECOVERY HOLD RESET]",
                            name,
                            "| PRICE", pe_ltp
                        )

                    memory[name]["pe_recovery_since"] = 0
                    memory[name]["pe_recovery_level"] = 0

                ce_recovery = (
                    memory[name]["ce_recovery_since"] > 0
                    and time.time() - memory[name]["ce_recovery_since"] >= 20
                    and ce_ltp >= memory[name]["ce_recovery_level"]
                )

                pe_recovery = (
                    memory[name]["pe_recovery_since"] > 0
                    and time.time() - memory[name]["pe_recovery_since"] >= 20
                    and pe_ltp >= memory[name]["pe_recovery_level"]
                )
...
if token:
    print("BOT READY")
    send("🟢 OptionFlowAI Bot Started Successfully")
    try:
        scanner(token)
